...
for i in range(0,N):
    ...
    rp[i] = float(header[1])
    Qp[i] = float(header[2])
    mig[i] = float(header[4]) + float(header[4])/4. #damping time too
...
if arg2==11:
    ...
    if tide_delay > 1.:
        plt.plot([tide_delay, tide_delay], [1.99,2.01], label='tides turned on now!', color='black', linewidth=2)
    plt.plot([mig[1], mig[1]], [1.99,2.01], label='Migration stops!', color='black', linewidth=2, ls='--')
elif arg2==12:
    for i in range(0,N-1):
        q=data[i+1::N]
        length=len(q[:,8])
