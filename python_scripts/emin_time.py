...
while i < N_sys:
    ...
    for k in xrange(0,N_T):
        T = T_arr[k]
        num1 = ((delta + 2.0)/2.0)**(2./3.)
        if T == -1: #T -> infinity case
            ecc = math.sqrt(math.log(mig_out*num1))
        else:
            X_in = T/tau_e
            ecc = (math.log(mig_out*num1)/(1. - math.exp(-2*X_in)))**0.5
        e_i[i/2,k] = ecc
    i += 2
