...
#systems = np.delete(systems,[10,11,16,17,26,27,28,29,30,31])
...
D = np.zeros(0)
Dnumarr = np.zeros(0)
dD = np.zeros(0)    #Difference in Delta between numerics & theory
dafin = np.zeros(0) #Difference in final 'a' between numerics & theory (inner planet)
dafout = np.zeros(0)#Difference in final 'a' between numerics & theory (outer planet)
daout = np.zeros(0) #Difference in 'a' between beginning & end, measure of res repuls (outer planet, numerics)
...
while i < N_sys:
    ...
    while exit != 1:    #now get initial values, before tides are turned on.
        ...
        if tt > lower and tt < upper:
            ...
            D = np.append(D, Dth)
            Dnum = (a_fout/a_fin)**1.5 - 2.0              #Period ratio, delta_numerical
...
print 'median migration of outer planet', np.median(daout)
